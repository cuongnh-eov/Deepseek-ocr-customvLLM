# configs/config.py (hoặc app/core/config.py)

import os

# ═══════════════════════════════════════════════════════════════════
# MODEL SETTINGS
# ═══════════════════════════════════════════════════════════════════
BASE_SIZE = 1024
IMAGE_SIZE = 640
CROP_MODE = True
MIN_CROPS = 2
MAX_CROPS = 6
MAX_CONCURRENCY = 32
NUM_WORKERS = 64
PRINT_NUM_VIS_TOKENS = False
SKIP_REPEAT = True

# ✅ SỬA: Đọc từ biến môi trường, fallback về giá trị local khi dev
MODEL_PATH = os.getenv('MODEL_PATH', '/home/cuongnh/PycharmProjects/TTTS_01/DeepSeek-OCRR')

# ═══════════════════════════════════════════════════════════════════
# INPUT/OUTPUT PATHS
# ═══════════════════════════════════════════════════════════════════
INPUT_PATH = os.getenv('INPUT_PATH', '/home/cuongnh/OCR_docs/data/1_kts_2025_10_23_23347e6_vi_baocaotaichinh_q1_2026.pdf')
OUTPUT_PATH = os.getenv('OUTPUT_PATH', './outputs')
UPLOAD_PATH = os.getenv('UPLOAD_PATH', './uploads')

# ═══════════════════════════════════════════════════════════════════
# MINIO SETTINGS
# ═══════════════════════════════════════════════════════════════════
# ✅ SỬA: Đọc từ biến môi trường
MINIO_ENDPOINT = os.getenv('MINIO_ENDPOINT', 'http://localhost:9000')
MINIO_ACCESS_KEY = os.getenv('MINIO_ACCESS_KEY', 'rag_flow')
MINIO_SECRET_KEY = os.getenv('MINIO_SECRET_KEY', 'infini_rag_flow')
MINIO_BUCKET_NAME = os.getenv('MINIO_BUCKET_NAME', 'ocr-results')

# ═══════════════════════════════════════════════════════════════════
# DATABASE & MESSAGE QUEUE
# ═══════════════════════════════════════════════════════════════════
DATABASE_URL = os.getenv('DATABASE_URL', 'postgresql+psycopg2://ocr_cuong:ocr_cuong@localhost:5432/ocr_cuong_db')

# RabbitMQ - clean up URL
raw_rabbit = os.getenv('RABBIT_URL', 'amqp://guest:guest@localhost:5672/')
RABBIT_URL = raw_rabbit.strip().rstrip('/') + '/'

# Redis
REDIS_URL = os.getenv('REDIS_URL', 'redis://:infini_rag_flow@127.0.0.1:6379/0')

# ═══════════════════════════════════════════════════════════════════
# OTHER SETTINGS
# ═══════════════════════════════════════════════════════════════════
QUEUE_NAME = os.getenv('QUEUE_NAME', 'ocr_jobs')
MAX_UPLOAD_MB = int(os.getenv('MAX_UPLOAD_MB', '200'))
PROMPT = '<image>\n<|grounding|>Convert the document to markdown.'
_IMAGE_TOKEN = "<image>"
CHUNK_SIZE = 40

# ═══════════════════════════════════════════════════════════════════
# TOKENIZER INITIALIZATION
# ═══════════════════════════════════════════════════════════════════
try:
    from transformers import AutoTokenizer
    TOKENIZER = AutoTokenizer.from_pretrained(MODEL_PATH, trust_remote_code=True)
    print(f"✅ Tokenizer loaded from: {MODEL_PATH}")
except Exception as e:
    TOKENIZER = None
    print(f"⚠️ Không load được Tokenizer DeepSeek: {e}. Worker sẽ chạy ở chế độ dự phòng.")