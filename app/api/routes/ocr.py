...
#     """
#     Tiếp nhận file PDF, lưu vào bộ nhớ đệm và đẩy vào hàng đợi Celery.
#     """
...
#     # Đẩy Job vào Celery - Đảm bảo PYTHONPATH đã được set để tìm thấy app.tasks
#     try:
...
#         db.commit()
#         raise HTTPException(status_code=500, detail="Không thể đẩy job vào hàng đợi RabbitMQ.")

#     return {
#         "job_id": job_id,
#         "status": JobStatus.QUEUED,
#         "message": "Tài liệu đã được tiếp nhận và đang chờ xử lý."
#     }
...
#     """
#     Kiểm tra trạng thái xử lý, tiến độ và các chỉ số GPU liên quan.
#     """
...
#     """
#     Liệt kê danh sách lịch sử tất cả các job OCR.
#     """
#     return db.query(OCRJob).order_by(OCRJob.created_at.desc()).all()


# @ocr_router.delete("/documents/{id}")
# def delete_document(id: str, db: Session = Depends(get_db)):
#     """
#     Xóa tài liệu: Xóa bản ghi DB + Xóa file PDF gốc + Xóa toàn bộ thư mục kết quả.
#     """
#     job = db.query(OCRJob).filter(OCRJob.job_id == id).first()
#     if not job:
#         raise HTTPException(status_code=404, detail="Tài liệu không tồn tại.")

#     # 1. Xóa thư mục kết quả (MD, JSON, images)
...
#     """
#     Tải file Markdown kết quả sau khi xử lý thành công.
#     """
#     job = db.query(OCRJob).filter(OCRJob.job_id == id).first()
#     if not job or job.status != JobStatus.SUCCESS: 
#         status_info = job.status if job else "N/A"
#         raise HTTPException(status_code=400, detail=f"Kết quả chưa sẵn sàng (Trạng thái: {status_info})")

#     file_full_path = os.path.join(OUTPUT_PATH, id, f"{id}.md")

#     if not os.path.exists(file_full_path):
#         raise HTTPException(status_code=404, detail="File Markdown không tồn tại trên hệ thống lưu trữ.")

#     return FileResponse(
#         path=file_full_path, 
#         filename=f"OCR_Result_{id}.md",
...
# --- IMPORT SCHEMAS ---
# Import các class từ file schemas bạn đã viết
